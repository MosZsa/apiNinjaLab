...
@router.post("/ingredients", response=IngredientOut, tags=["Ингредиенты"])
...
@router.get("/ingredients", response=List[IngredientOut], tags=["Ингредиенты"])
...
@router.get("/ingredients/{ingredient_id}", response=IngredientOut, tags=["Ингредиенты"])
def get_ingredient(request, ingredient_id: int):
    obj = get_object_or_404(Ingredient, id=ingredient_id, user=request.user)
    return IngredientOut.from_orm(obj)

@router.put("/ingredients/{ingredient_id}", response=IngredientOut, tags=["Ингредиенты"])
...
@router.delete("/ingredients/{ingredient_id}", tags=["Ингредиенты"])
...
@router.post("/recipes", response=RecipeOut, tags=["Рецепты"])
...
@router.get("/recipes", response=List[RecipeOut], tags=["Рецепты"])
def list_recipes(
    request,
    name: str = None,
...
@router.get("/recipes/{recipe_id}", response=RecipeResult, tags=["Рецепты"])
...
@router.put("/recipes/{recipe_id}", response=RecipeOut, tags=["Рецепты"])
...
@router.delete("/recipes/{recipe_id}", tags=["Рецепты"])
